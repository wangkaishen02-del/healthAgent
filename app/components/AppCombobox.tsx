...
export default function AppCombobox({ value, options, onChange, onSelect, ariaLabel, placeholder = "请输入关键词", disabled = false, invalid = false }: {
  value: string;
  ...
  return <div className={`config-select app-combobox ${disabled ? "disabled" : ""}`} ref={rootRef}>
    ...
    <div className={`config-select-menu ${open ? "" : "hidden"}`} role="listbox" aria-label={`${ariaLabel}选项`}>
      {filtered.map((item) => <button key={item.value} type="button" role="option" aria-selected={item.label === value} className={item.label === value ? "active" : ""} onClick={() => { onChange(item.label); onSelect?.(item); setOpen(false); }}><span>{item.label}</span></button>)}
