import { BadRequestException, Body, ConflictException, Controller, Delete, Get, Inject, InternalServerErrorException, NotFoundException, Post, Put, Query } from "@nestjs/common";
...
export class CalculationController {
  ...
  async create(@Body() body: unknown) {
    ...
    try { return await this.calculation.create(input); } catch (error) { throwMutationError(error); }
  }
  ...
  async update(@Body() body: unknown) {
    ...
    try {
      const result = await this.calculation.update((body as { id: string }).id, input);
