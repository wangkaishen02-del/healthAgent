...
export async function queryUnderwritingDb(input: QueryUnderwritingInput) {
  ...
  const matches = rows.map((item) => ({ policy: { id: item.policy.id, policyNo: item.policy.policyNo, policyName: item.policy.policyName ?? undefined, applicantName: item.policy.applicantName, policyStatus: item.policy.policyStatus, effectiveDate: dateOnly(item.policy.effectiveDate), expiryDate: dateOnly(item.policy.expiryDate) }, policyInsured: { id: item.id, effectiveDate: dateOnly(item.effectiveDate), expiryDate: dateOnly(item.expiryDate), coveragePlanId: item.coveragePlanId ?? undefined }, insuredPerson: { id: item.insuredPerson.id, insuredNo: item.insuredPerson.insuredNo, name: item.insuredPerson.name, gender: item.insuredPerson.gender ?? undefined, birthDate: optionalDate(item.insuredPerson.birthDate), idType: item.insuredPerson.idType ?? undefined, idNo: item.insuredPerson.idNo ?? undefined, phone: item.insuredPerson.phone ?? undefined }, coveragePlan: item.coveragePlan ? { id: item.coveragePlan.id, planCode: item.coveragePlan.planCode, planName: item.coveragePlan.planName } : undefined }));
