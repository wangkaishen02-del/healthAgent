...
保存、撤件、提交操作固定在页面右下方，不随内容滚动。新建状态按钮显示“保存立案”，打开已立案案件后显示“保存修改”；新建立案成功后顶部弹出案件号提示，同时刷新案件列表并清空表单回到新建状态。已保存且未提交的案件可撤件或提交，提交和撤件后只读。页面内所有下拉组件均使用项目统一的自定义下拉组件。
...
受理立案页面已经注册到通用 Agent 页面注册中心。Agent 可以按保单号和证件号锁定人员、打开或新建案件、操作全部关系人字段与资料复用开关、筛选和维护事件、选择影像分类、删除影像，以及保存、撤件和提交；注册信息描述的是可复用的页面能力，不在提示词中绑定具体案件。出于本地文件权限与安全考虑，影像文件仍由用户手动选择上传。
