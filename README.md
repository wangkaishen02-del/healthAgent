...
- 智能助手默认通过已配置的 DeepSeek 自动检查案件、汇总待办、打开页面、填写条件和执行查询
- Agent 可只读检查指定案件的当前环节、资料数量、OCR 状态、理算结果、风险提示和当前角色可执行的下一步
- Agent 可实时汇总受理、录入、理算、审核、结案、撤件各环节案件量及 OCR 队列状态
...
- 菜单：综合查询、理赔处理、理赔配置
- 页面：保单查询与详情、案件查询、受理立案、录入与理算、审核结案、保单理算配置
- 页面能力：查询条件、详情导航、关系人/事件/账单/疾病/备注/影像区域，以及由业务状态控制的保存和流转动作
- 后台只读能力：承保关系查询、案件定位、案件体检和工作队列汇总
...
Agent 独立服务的收益、前置条件、量化触发标准和目标边界见 [`docs/agent-service-evaluation.md`](docs/agent-service-evaluation.md)。当前结论是保持 NestJS 内模块化部署，先完成可拆边界，不立即增加容器。

案件状态、正向流转、逐级回退、数据锁定和并发规则见 [`docs/claim-state-machine.md`](docs/claim-state-machine.md)。
...
服务端单次计划修正最多执行 5 轮，前端对连续完全相同的工具计划执行熔断，防止模型异常时重复操作。

## 当前 LLM 工具

LLM目前可用工具分为三类：

注册信息查询接口（仅供前端和诊断使用，不提供给 LLM）：
...
导航和页面注册信息由系统直接提供给 LLM，不需要模型调用发现工具。`get_navigation_registry`、`get_menu_pages` 和 `get_page_registry` 均不再作为 LLM 工具提供。
...
列表行操作打开保单详情抽屉后，系统会切换到注册页面 `policy_detail` 并加载其运行时可用注册信息。下一轮只传递该页面的注册能力，不传递当前页面路径，也不继续传递 `policy_query` 的页面注册信息。
...
DeepSeek 请求使用非 thinking 模式和 JSON 输出，以减少页面操作 Agent 的响应时间。发送到外部模型前，服务端会裁掉文件内容、OCR 全文、超长字符串和过长列表，并把姓名、证件号、手机号、银行卡号、邮箱、诊断和地址替换为本次请求内有效的占位符；模型返回页面动作后再在服务端内存中恢复原值。
...
每次 LLM 调用默认只向控制台和 `logs/assistant-llm.log` 记录运行 ID、轮次、模型、耗时、字符数和内容摘要，不写入提示词或模型输出正文。`ASSISTANT_LOG_LEVEL=off` 可完全关闭，`redacted` 仅用于本地排障并仍会掩码常见个人信息；生产环境应保持默认的 `metadata`。

```text
====================================================================================
[assistant-llm] 2026-07-11T...Z | run=1a2b3c4d | turn=1 | OUTPUT
------------------------------------------------------------------------------------
duration=18234ms | output_chars=186 | sha256=9cbd12f9b04e5a71
====================================================================================
```
...
影像资料支持理赔申请书、身份证明、病历、发票、银行卡及其他分类，允许上传 JPG、PNG、WebP 和 PDF，单件最大 10MB。文件正文保存在本机影像目录，后续可迁移到对象存储。每次上传会立即创建 PostgreSQL 持久化 OCR 任务，由后台顺序调用本机 RapidOCR 服务；原文、逐行坐标与置信度、页码、文档类型和结构化字段均写入数据库，失败任务自动退避重试。数据库表不创建数据库外键，关联完整性由应用服务与事务校验。
...
安装依赖并启动 PostgreSQL、OCR 后，一条命令同时启动 Next.js 页面和 NestJS API：
...
- OCR API：`http://127.0.0.1:18080/health`
